...
// Global test dispatcher & manager used to ensure teams are registered
static StaffMember* g_testDispatcher = nullptr;
static Gardener* g_testGardener = nullptr;
static Cashier* g_testCashier = nullptr;
static StaffManager* g_testManager = nullptr;
...
void waitForUser()
{
    // Skip prompts in non-interactive environments (CI/CD)
...
void testBridgePattern()
{
    ...
    // -----------------------------------------------------------------
    // Safety setup: ensure a minimal staff dispatcher with Greenhouse team
    // is available so any lifecycle notifications that dispatch commands
    // won't fail due to a missing "Greenhouse" team. This mirrors the
    // runtime setup used by the integrated app and keeps tests robust.
    // -----------------------------------------------------------------
    // create or reuse the global test dispatcher and handlers
    g_testDispatcher = new StaffMember();
    // create one gardener (greenhouse) and one cashier (sales) and register
    g_testGardener = new Gardener();
    g_testCashier = new Cashier();
    g_testGardener->setManager(g_testDispatcher);
    g_testCashier->setManager(g_testDispatcher);
    g_testDispatcher->registerTeam("Greenhouse", g_testGardener);
    g_testDispatcher->registerTeam("Sales", g_testCashier);

    // Create a StaffManager that will be used by plants if they are attached
    g_testManager = new StaffManager(g_testDispatcher);
...
int main()
{
    ...
    cout << string(80, '=') << endl;
    // Cleanup the test dispatcher and teams created for safety
    if (g_testManager) { delete g_testManager; g_testManager = nullptr; }
    if (g_testGardener) { delete g_testGardener; g_testGardener = nullptr; }
    if (g_testCashier) { delete g_testCashier; g_testCashier = nullptr; }
    if (g_testDispatcher) { delete g_testDispatcher; g_testDispatcher = nullptr; }
